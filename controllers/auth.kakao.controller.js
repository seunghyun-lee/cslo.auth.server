...
const kakao = require('../modules/auth.kakao.module');
const gameServerModule = require('../modules/game.server.module');

const AuthKakaoController = {

    authSetKakao: function(req, res, next) {
        ...
        if(!commonFunc.ObjectKeyCheck(bodyData,  ['playerId', 'accessToken', 'IDPCode', 'IDPUserID', 'redis' ])) {
        ...
        kakao.accessTokenValidate(bodyData)
        .then(tokenValidated=> {
            if(tokenValidated.desc){                
                throw error(errorCode.InvalidToken, tokenValidated); // { "desc": "zat is invalid." }
            } else {
                bodyData.tokenValidated = tokenValidated;
            }
            // DB에 카카오 계정(게스트 or PlayerId) 조회 및 등록.
            return kakao.accountSetUserAuthKakao(bodyData);
        })
        .then(userData=> {
...
module.exports = AuthKakaoController;
